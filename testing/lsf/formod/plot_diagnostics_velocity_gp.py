...
u = results['u']                        # km/s
shape_coeffs = results['shape_coeffs']  # (N_U_INDUCING, N_X_INDUCING): the 2D GP's inducing grid D
u_inducing = results['u_inducing']
x_inducing = results['x_inducing']
...
# Must match lsf_reconstruction_velocity.py's section 7 exactly.
SHAPE_U_LENGTH_SCALE_FACTOR = 0.5
SHAPE_X_LENGTH_SCALE = 3000
_JITTER = 1e-3
...
def departure_at(x_pos, sigma):
    """ The departure curve (over the full u grid) at a single position
        x_pos, given the LSF width sigma there -- the GP's own
        conditional-mean interpolation from the inducing grid D, exactly
        as lsf_reconstruction_velocity.py's evaluate_departure does. """
    l_u = SHAPE_U_LENGTH_SCALE_FACTOR * sigma
    R_u = unit_rbf(u_inducing, u_inducing, l_u) + _JITTER * np.eye(len(u_inducing))
    W_u_full = unit_rbf(u, u_inducing, l_u) @ np.linalg.inv(R_u)
    R_x = unit_rbf(x_inducing, x_inducing, SHAPE_X_LENGTH_SCALE) + _JITTER * np.eye(len(x_inducing))
    w_x = unit_rbf(np.array([x_pos]), x_inducing, SHAPE_X_LENGTH_SCALE) @ np.linalg.inv(R_x)
    return (W_u_full @ shape_coeffs @ w_x.T).flatten()
...
for x_pos, colour in zip(_x_pos_array, _colours):
    ...
    lsf = gaussian_mean(u, sigma) + departure_at(x_pos, sigma)
...
for x_pos, colour in zip(_x_pos_array, _colours):
    sigma = np.interp(x_pos, line_position, line_width)
    departure_curve = departure_at(x_pos, sigma)
    mask = data_constrained_mask(x_pos, u)
...
for m in range(len(line_position)):
    ...
    ax.plot(u_data, flux[idx], '.', ms=4, alpha=0.85, color=colour)
u_fine = np.linspace(u.min(), u.max(), 2000)
for x_pos, colour in zip(_x_pos_array, _colours):
    sigma = np.interp(x_pos, line_position, line_width)
    lsf_native = gaussian_mean(u, sigma) + departure_at(x_pos, sigma)
...
# velocity knot, colour is the fitted departure value there. Unlike the
# earlier Chebyshev/Hermite versions, there's no natural "one curve per
# order" decomposition any more -- the departure is genuinely 2D, so this
# shows it directly rather than projecting it onto a 1D summary.
ax = axes[1, 1]
...
plt.colorbar(im, ax=ax, label='departure')
...
ax.plot(pixel, fwhm_grid, color='tab:orange', lw=1.3, label='FWHM(x)')
ax.errorbar(line_position, 2.355*np.exp(width_raw_target), 
...
ax.fill_between(pixel, fwhm_lower, fwhm_upper, color='tab:orange', alpha=0.3,
...
ax.set_ylim(1.5, 3.0)
ax_r = ax.secondary_yaxis('right', functions=(lambda x: SPEED_OF_LIGHT_KMS / x, lambda x: SPEED_OF_LIGHT_KMS / x))
# ax_r = ax.twinx()
