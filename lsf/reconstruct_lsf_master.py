...
# Per-kernel-type hyperparameter spec, one entry per NON-AMPLITUDE parameter,
# in the order _make_kernel below expects them: (name, rescale_by_x_range).
# rescale_by_x_range=True means the parameter is a LENGTH measured in the
# same units as x (so its prior mean, given in raw x units at the call site,
# needs the same /x_range normalisation length scales already got); False
# means the parameter is dimensionless and used as given.
#
# 'locallyperiodic' is new: ExpSquared(decay_length) * ExpSineSquared(period,
# gamma), the "locally periodic" construction from Duvenaud's kernel
# cookbook (https://www.cs.toronto.edu/~duvenaud/cookbook/) -- exactly
# periodic at lag=period (via ExpSineSquared), with that periodicity allowed
# to decay/drift over decay_length (via the ExpSquared envelope on top),
# rather than being forced exactly periodic over the WHOLE input range the
# way a bare ExpSineSquared alone would be. Three non-amplitude
# hyperparameters instead of one is why this needed generalising beyond the
# old fixed (log_amp, log_length) pair-per-component layout.
_KERNEL_PARAM_SPECS = {
...
def _component_kernel(kernel_type, log_subparams):
    """ Builds one component's kernel (no amplitude) from its log-space
        hyperparameters, in the order given by _KERNEL_PARAM_SPECS. """
...
def _component_offsets(kernel_types):
    """ Starting index, in the flat params vector, of each component
        (amplitude first, then its non-amplitude hyperparameters) -- needed
        now that components can have different lengths (1 param for a
        simple stationary kernel, 3 for locally-periodic). """
    offsets, offset = [], 0
...
def map_gp_fit(
    ...
    length_scale_priors: list,   # list of (mean_in_x_units, log_std), one
                                   ...
                                   # For a kernel with more than one non-
                                   # amplitude hyperparameter (currently only
                                   # 'locallyperiodic': length, period,
                                   # gamma), that component's entry is
                                   # instead a list of that many (mean,
                                   # log_std) tuples, in the order given by
                                   # _KERNEL_PARAM_SPECS[kernel_type] --
                                   # length and period in x units (get the
                                   # same /x_range normalisation as an
                                   # ordinary length-scale prior), gamma
                                   # dimensionless (used as given).
    ...
    # Per-component, per-hyperparameter (log_mean, log_std) priors. Each
    # entry of length_scale_priors is either a single (mean_in_x_units,
    # log_std) tuple -- the original, backward-compatible form, valid for
    # every kernel with exactly one non-amplitude hyperparameter (expsquared,
    # matern32, matern52) -- or, for a kernel with more than one (currently
    # only 'locallyperiodic', with 3: length, period, gamma), a list of that
    # many (mean, log_std) tuples in the order given by
    # _KERNEL_PARAM_SPECS[kernel_type]. Only length-like sub-parameters
    # (rescale=True in the spec) get the /x_range normalisation every other
    # length-scale prior in this function already receives; a dimensionless
    # one (gamma) is used exactly as given.
    component_sub_priors = []   # per component: list of (log_mean, log_std)
    ...
    # Interleaved as [log_amp_1, <component 1's sub-params...>, log_amp_2,
    # <component 2's sub-params...>, ...], matching _make_kernel's own
    # _component_offsets-based slicing -- generalises the old fixed
    # [log_amp_1, log_length_1, log_amp_2, log_length_2, ...] layout to
    ...
    # length_scale[k] is a plain float (pixel units) for a single-sub-param
    # kernel, same as always -- or, for a multi-sub-param kernel like
    # 'locallyperiodic', a {'length':..., 'period':..., 'gamma':...} dict
    # (length/period in pixel units, gamma dimensionless), so callers that
    # care about the individual hyperparameters can get at them by name
    # instead of guessing a flat-array position.
...
def poly_plus_gp_fit(
    ...
    x_grid: np.ndarray,
    poly_degree: int,
    length_scale_prior: tuple,   # (mean, log_std), OR -- for a kernel with
                                   # more than one non-amplitude
                                   # hyperparameter, e.g. 'locallyperiodic' --
                                   # a list of (mean, log_std) tuples; see
                                   # map_gp_fit's length_scale_priors for the
                                   # exact convention, forwarded unchanged
...
ENVELOPE_RESIDUAL_LENGTH_SCALE_PRIOR = (100, 1.0)  # (mean pixels, log-std);
                                                      ...
                                                      # Kept, unused, for an
                                                      # easy revert -- see
                                                      # ENVELOPE_KERNEL_TYPE.

# LOCALLY PERIODIC KERNEL for the envelope residual: ExpSquared(decay_length)
# * ExpSineSquared(period, gamma), the "locally periodic" construction from
# Duvenaud's kernel cookbook (https://www.cs.toronto.edu/~duvenaud/cookbook/).
# The ExpSineSquared factor enforces exact periodicity at lag=period; the
# ExpSquared factor lets that periodicity decay/drift over decay_length
# rather than being forced exactly periodic over the WHOLE order the way a
# bare ExpSineSquared alone would be -- appropriate if the envelope carries
# a genuinely periodic ripple (e.g. interference fringing, or a per-
# amplifier gain pattern) riding on top of the smooth blaze-like trend the
# polynomial already absorbs, rather than a strictly stationary residual
# that Matern32 (the previous default, kept above) was already a reasonable
# model for.
#
# All three priors below are STARTING POINTS, not validated constants --
# same caveat as SHAPE_KAPPA_SIGMA0/SIGMAF elsewhere in this script:
#   - period: centred on n_pixels/8, motivated by ESPRESSO's known 8-
#     amplifier readout segmentation (see the note on window 3's 1/8-
#     detector boundary lines) -- worth checking against the actual
#     envelope residual (e.g. a Lomb-Scargle periodogram, the same
#     diagnostic already used to set WIDTH_LENGTH_SCALE_PRIOR) rather than
#     trusting this blindly.
#   - decay_length: centred at 3 periods, i.e. the periodicity is assumed to
#     persist over a few cycles before drifting -- if the true ripple is
#     either near-perfectly stable across the whole order or only
#     coherent for a single cycle, this prior is centred in the wrong place
#     and should be tightened/moved once checked against real data.
#   - gamma: controls how sharply peaked (large gamma) vs broadly sinusoidal
#     (small gamma) each cycle is; 2.0 is a generic, unvalidated mid-range
#     guess with a loose prior since there's no prior information on this
#     yet.
ENVELOPE_PERIODIC_DECAY_LENGTH_PRIOR = (3 * n_pixels / 8, 0.6)  # (mean pixels, log-std)
ENVELOPE_PERIOD_PRIOR = (n_pixels / 8, 0.3)                       # (mean pixels, log-std)
ENVELOPE_GAMMA_PRIOR = (2.0, 1.0)                                  # (mean, log-std); dimensionless
ENVELOPE_LOCALLY_PERIODIC_PRIOR = [ENVELOPE_PERIODIC_DECAY_LENGTH_PRIOR,
                                     ENVELOPE_PERIOD_PRIOR, ENVELOPE_GAMMA_PRIOR]

ENVELOPE_KERNEL_TYPE = 'locallyperiodic'  # was 'matern32' (still defined
                                             # above, with its own prior, to
                                             # revert easily: set this back
                                             # to 'matern32' and pass
                                             # length_scale_prior=
                                             # ENVELOPE_RESIDUAL_LENGTH_SCALE_PRIOR
                                             # at the fit_envelope_background
                                             # call site below instead of
                                             # ENVELOPE_LOCALLY_PERIODIC_PRIOR)
...
def background_design_row(x, e):
    ...
    return np.column_stack([np.ones_like(x_tilde)] +
                            [x_tilde**p * e for p in range(0, BACKGROUND_POLY_ORDER )])
...
def fit_envelope_background(centre_positions):
    ...
    gp_fit = poly_plus_gp_fit(env_peak_pixel, env_peak_flux, env_peak_flux_err,
                                ...
                                length_scale_prior=ENVELOPE_LOCALLY_PERIODIC_PRIOR,
    ...
    # gp_length_scale is a plain float for a single-hyperparameter kernel
    # (e.g. matern32), or a {'length','period','gamma'} dict for
    # 'locallyperiodic' -- print whichever this run actually used rather
    # than assuming the old scalar form and breaking on the new one.
    _env_gp_ls = gp_fit['gp_length_scale']
    if isinstance(_env_gp_ls, dict):
        _env_gp_ls_str = (f"decay_length={_env_gp_ls['length']:.1f} pix, "
                           f"period={_env_gp_ls['period']:.1f} pix, "
                           f"gamma={_env_gp_ls['gamma']:.3f}")
    else:
        _env_gp_ls_str = f"length scale = {_env_gp_ls:.1f} pix"
    print(f"  envelope/background: envelope residual GP ({_env_gp_ls_str}), "
          f"background residual GP length scale = "
...
def get_half_window(y_axis,plot=False,minimum=8):
...
HALF_WINDOW = get_half_window(flux_new,plot=False)          # still in PIXELS -- how many detector pixels around
                          # each line's centre go into its fitting window
print(f"{HALF_WINDOW=:d} pixels")
_SAFETY_MARGIN = 1.8     # grid range set wider than the typical scale alone
                          # would need, so it still covers the fitting window
                          # wherever the local scale is larger than typical

VELOCITY_HALF_RANGE = HALF_WINDOW * _v_per_pixel_typical   # km/s
